...
export default function ShowChart({ value = 40, coin = 'bitcoin' }) {
    const [apiData, setApiData] = useState([])
    const url = `https://api.coingecko.com/api/v3/coins/${coin}/market_chart?vs_currency=usd&days=${value}`
    console.log(url)
    ...
    const data = {
        labels: volumeLabels?.slice(0, value),
        datasets: [
            {
                label: '',
                data: priceData,
